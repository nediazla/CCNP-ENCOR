...
La Figura 9-18 muestra las redes del Área 1 siendo resumidas en el ABR dentro del prefijo agregado 10.1.0.0/18. Si el enlace 10.1.12.0/24 falla, todos los routers en el Área 1 aún ejecutan el cálculo SPF, pero los routers en el Área 0 no se ven afectados porque las redes 10.1.13.0/24 y 10.1.34.0/24 no son conocidas fuera del Área 1.

![](img/20260113114808.png)

**Resumen Interárea**

El resumen interárea reduce la cantidad de LSAs tipo 3 que un ABR anuncia hacia un área cuando recibe LSAs tipo 1. El rango de resumen de red está asociado con un área de origen específica para los LSAs tipo 1.

Cuando un LSA tipo 1 dentro del rango de resumen alcanza al ABR desde el área de origen, el ABR crea un LSA tipo 3 para la red resumida. El ABR suprime la generación y el anuncio de LSAs tipo 3 más específicos hacia las áreas vecinas. El resumen interárea no afecta a los LSAs tipo 1 en el área de origen.

La Figura 9-19 muestra 15 LSAs tipo 1 (172.16.1.0/24 a 172.16.15.0/24) siendo resumidos en un solo LSA tipo 3 (la red 172.16.0.0/20).

![](img/9-19.jpg)

**Métricas de Resumen**

En el momento de redactar este texto, los routers IOS XE establecen la métrica predeterminada para el **LSA de resumen** como la métrica más baja asociada con un LSA, según las directrices del RFC 1583. Sin embargo, la métrica de resumen puede configurarse estáticamente como parte de la configuración. En la Figura 9-20, R1 resume prefijos con varios costos de ruta. El prefijo 172.16.3.0/24 tiene la métrica más baja, por lo que esa métrica se utiliza para la ruta resumida.

![](img/9-20.jpg)

OSPF se comporta de forma similar a Enhanced Interior Gateway Routing Protocol (EIGRP) y verifica cada prefijo dentro del rango de resumen cuando se agrega o elimina un LSA de tipo 1 coincidente. Si se encuentra disponible una métrica más baja, el LSA de resumen se anuncia con la nueva métrica; si la métrica más baja se elimina, se identifica una métrica nueva y más alta, y se anuncia un nuevo LSA de resumen con la métrica más alta.

**Configuración de la Sumarización Interárea**

Para definir el rango de sumarización y el área asociada, se utiliza el comando  
`area area-id range network subnet-mask [advertise | not-advertise] [cost metric]`  
dentro del proceso OSPF en el ABR. El comportamiento predeterminado es anunciar el prefijo resumido, por lo que la palabra clave `advertise` no es necesaria. Agregar la palabra clave `cost metric` al comando establece de forma estática la métrica de la ruta resumida.

La Figura 9-21 muestra un ejemplo de topología en el que R1 está anunciando las redes 172.16.1.0/24, 172.16.2.0/24 y 172.16.3.0/24.

![](img/9-21.jpg)

El **Ejemplo 9-7** muestra la tabla de enrutamiento en R3 antes de la sumarización. Observe que las rutas 172.16.1.0/24, 172.16.2.0/24 y 172.16.3.0/24 están todas presentes.

**Ejemplo 9-7:** Tabla de Enrutamiento Antes de la Sumarización de Rutas Interárea OSPF
```
...
Dado que las rutas componentes 172.16.1.0/24, 172.16.2.0/24 y 172.16.3.0/24 están siendo anunciadas en el Área 0, R2 resume estas rutas en una única ruta de resumen 172.16.0.0/16. El **Ejemplo 9-8** proporciona la configuración de R2 para la sumarización interárea hacia una ruta agregada 172.16.0.0/16. Se agrega un costo estático de 45 a la ruta resumida para reducir la carga de CPU si cualquiera de las tres redes fluctúa.

**Ejemplo 9-8:** Configuración de Sumarización de Rutas Interárea en R2
```
router ospf 1
...
El **Ejemplo 9-9** muestra la tabla de enrutamiento de R3 para verificar que las rutas componentes más pequeñas fueron suprimidas, mientras que la ruta de resumen fue agregada. Observe que la métrica del camino es 46, mientras que anteriormente la métrica para la red 172.16.1.0/24 era 3.

**Ejemplo 9-9:** Tabla de Enrutamiento Después de la Sumarización de Rutas Interárea OSPF
```
R3# show ip route ospf | begin Gateway
Gateway of last resort is not set
...
El ABR que realiza la **sumarización interárea** instala una **ruta de descarte** (discard route), es decir, una ruta hacia la interfaz **Null0** que coincide con el rango de red resumido. Las rutas de descarte evitan bucles de enrutamiento cuando partes de la red resumida no tienen una ruta más específica en la RIB. La **distancia administrativa (AD)** de la ruta de descarte de resumen OSPF para redes internas es **110**, y para redes externas es **254**.

El **Ejemplo 9-10** muestra la ruta de descarte en R2 para el prefijo **172.16.0.0/16**.

**Ejemplo 9-10:** Descartar una Ruta para Prevención de Bucles
```
...
El **filtrado de rutas** es un método para identificar selectivamente rutas que se anuncian o se reciben desde routers vecinos. El filtrado de rutas puede usarse para manipular flujos de tráfico, reducir el uso de memoria o mejorar la seguridad.

El filtrado de rutas con protocolos de enrutamiento de vector distancia es directo, porque las rutas se filtran cuando las actualizaciones se anuncian a los vecinos aguas abajo. Sin embargo, con protocolos de estado de enlace como OSPF, cada router en un área comparte una copia completa de la base de datos de estado de enlace. Por lo tanto, el filtrado de rutas generalmente ocurre cuando las rutas entran al área en el **ABR**.

Las siguientes secciones describen tres técnicas para filtrar rutas con OSPF.

## Filtrado mediante Sumarización

Uno de los métodos más sencillos para filtrar rutas es usar la palabra clave **not-advertise** durante la sumarización de prefijos. El uso de esta palabra clave evita la creación de cualquier **LSA tipo 3** para las redes dentro de ese rango, haciendo que las rutas componentes más pequeñas sean visibles únicamente dentro del área donde se originan.

La estructura completa del comando es:

```
area area-id range network subnet-mask not-advertise
```

bajo el proceso OSPF.

Si retomamos la **Figura 9-21**, donde R1 está anunciando **172.16.1.0/24**, **172.16.2.0/24** y **172.16.3.0/24**, vemos que R2 puede filtrar cualquiera de las **LSA tipo 1** que se generan en el Área 12 para que no sean anunciadas hacia el Área 0. La configuración se muestra en el **Ejemplo 9-11**.

**Ejemplo 9-11:** Configuración de R2 para Filtrado mediante Sumarización
```
...
El **Ejemplo 9-12** muestra la tabla de enrutamiento de R3 después de que la configuración de filtrado se haya aplicado en R2. La ruta **172.16.2.0/24** es filtrada y eliminada del Área 0. Si se configurara un rango de red mayor, se filtrarían más rutas componentes.

**Ejemplo 9-12:** Verificación de la Eliminación de 172.16.2.0 del Área 0
```
R3# show ip route ospf | begin Gateway
Gateway of last resort is not set

     10.0.0.0/8 is variably subnetted, 3 subnets, 2 masks
O IA 10.12.1.0/24 [110/3] via 10.23.1.2, 00:02:24, GigabitEthernet0/0
     172.16.0.0/24 is subnetted, 2 subnets
O IA 172.16.1.0 [110/4] via 10.23.1.2, 00:00:17, GigabitEthernet0/0
O IA 172.16.3.0 [110/4] via 10.23.1.2, 00:00:17, GigabitEthernet0/0
```

## Filtrado por Área

Aunque el filtrado mediante sumarización es sencillo, es limitado en su capacidad. Por ejemplo, en la **Figura 9-22**, si la ruta **172.16.1.0/24** debe estar presente en el Área 0 pero eliminada del Área 34, no es posible filtrar la ruta usando sumarización.

Otros diseños de red requieren filtrar rutas OSPF basándose en otros criterios. OSPF admite el filtrado de **LSA tipo 3** durante la generación. Esto permite que la ruta original se instale en la **LSDB** del área de origen, mientras que la ruta puede filtrarse en la RIB del ABR. El filtrado puede ocurrir en cualquiera de las direcciones en el ABR. La **Figura 9-23** demuestra el concepto.

![](img/9-22.jpg)

![](img/9-23.jpg)

La **Figura 9-24** amplía la topología de ejemplo y demuestra que el **ABR** puede filtrar rutas tanto cuando se anuncian **hacia fuera de un área** como cuando **entran a un área**. R2 es capaz de filtrar rutas (LSA) **salientes** cuando abandonan el Área 12 o **entrantes** cuando ingresan al Área 0. Además, R3 puede filtrar rutas cuando salen del Área 0 o entran al Área 34. La misma lógica se aplica con rutas anunciadas en la dirección opuesta.

![](img/9-24.jpg)

El **filtrado de áreas OSPF** se realiza utilizando el comando:

```
area area-id filter-list prefix prefix-list-name {in | out}
```

en el **ABR**. Supongamos que R1 anuncia las rutas **172.16.1.0/24**, **172.16.2.0/24** y **172.16.3.0/24**. R2 está configurado para filtrar la ruta **172.16.1.0/24** cuando entra al **Área 0**, y R3 está configurado para filtrar la ruta **172.16.2.0/24** cuando sale del **Área 0**. El **Ejemplo 9-13** proporciona la configuración necesaria para R2 y R3.

**Ejemplo 9-13:** Configuración del filtrado de áreas OSPF
```
R2
...
El **Ejemplo 9-14** muestra la tabla de enrutamiento en R3 donde la ruta **172.16.1.0/24** ha sido filtrada de todos los routers del **Área 0**. La ruta **172.16.2.0/24** ha sido filtrada de todos los routers del **Área 34**. Esto verifica que el filtrado de áreas fue exitoso para las rutas que entran al backbone y las que salen del backbone.

**Ejemplo 9-14:** Verificación del filtrado de áreas OSPF
```
R3# show ip route ospf | begin Gateway
Gateway of last resort is not set
...
O IA 10.23.1.0/24 [110/4] via 10.34.1.3, 00:19:41, GigabitEthernet0/0
